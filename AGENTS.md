# Makii Web — Agente del Proyecto Web

> Este archivo define las reglas, patrones y mejores prácticas para el desarrollo de la aplicación web de Makii (landing page + panel de administración).
...
- Sincronizar tipos con los DTOs del backend (`makii-backend/AGENTS.md`).
...
- **Roles sin acceso web**: `WAITRESS`, `KITCHEN` (usan la app móvil).
...
1. Delegar primero a `makii-backend` usando su `AGENTS.md`.
...
**Referencia del backend:** `../makii-backend/AGENTS.md`
...
pnpm run build            # Build de producción (static export)
pnpm run start            # Iniciar servidor de producción (sirve out/)
...
- **Mismos tokens** que la app móvil (`makii-app/DESIGN.md`) para coherencia visual.
...
*[2026-06-22] Corregidas referencias: makiibacknest → makii-backend, makiiapp → makii-app. Rol WAITER → WAITRESS.*

*[2026-06-23] Rebranding completo: zitty → makii. Renombradas carpetas, package.json, layout title, landing/about/contact/partners, login, forgot-password, Navbar, Footer, AdminSidebar, AppMockupMenu, AppMockupTables. Email: hola@makii.com.co. Dominio objetivo: makii.com.co.*
